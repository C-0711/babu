...
def _puffer_tabelle(conn) -> None:
    """Die Puffertabelle, falls sie noch nicht da ist.

    Steht in Postgres-Schreibweise da und wird für SQLite übersetzt — der
    Dialekt kommt von der Verbindung, nicht aus der Umgebung: die Tests
    dieses Moduls reichen eine nackte `sqlite3`-Verbindung herein, auch
    wenn der Rest der Suite gerade gegen Postgres läuft. Dieselbe Tabelle
    liegt in `migrations/0001_initial.sql`; hier bleibt sie stehen, weil
    der Puffer auch dann greifen muss, wenn ihn keiner vorbereitet hat.
    """
    conn.execute(db.ddl(
