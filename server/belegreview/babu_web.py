...
# Briefkopf und Marketing (`/api/marke*`, `/api/marketing*`) liegen seit dem
# 08.09.2026 ebenfalls als eigenes Blatt daneben — siehe `marke_routen`. Der
# Router dort trägt KEIN Präfix, weil es zwei Adressfamilien sind und
# `/api/marke` selbst unter einem Präfix der leere Pfad wäre; die Pfade
# stehen deshalb vollständig an den Routen und sind Zeichen für Zeichen die
# alten. Innerhalb der Datei ist die Reihenfolge Absicht:
# `/api/marketing/entwerfen` steht vor `/api/marketing/{schluessel}`.
